...
export default function DashboardComponent() {
  ...
  useEffect(() => {
    ...
    const fetchServices = async () => {
      try {
        const res = await fetch("/api/services?limit=5");
        const data = await res.json();
        if (res.ok) {
          setServices(data);
          // These would need to be implemented in your backend
          setTotalServices(data.length); // Temporary - should come from backend
          setLastMonthServices(0); // Temporary - should come from backend
        }
      } catch (error) {
